...
export const formatErrorGenerator = (formatErrorOptions) => {
  ...
  let {logger, publicDataPath, hooks, showLocations, showPath, hideSensitiveData, transformFunc} = actualOptions;
  ...
  return function formatError (graphqlError) {
    ...
    if (err === undefined || !err.isBoom) {
      ...
      if (transformFunc && _.isFunction(transformFunc)) {
        err = transformFunc(err)
      } else {
